...
## First-Prize Reviewer Path

For a short review, read these four artifacts in order:

1. `docs/reviewer_quickstart.md` — exact commands, expected results, and artifact map.
2. `final_report/nutshell_cache_report.pdf` — polished final competition report.
3. `docs/ai_collaboration_report.md` — AI/human split, prompt strategy, manual fixes, and stage evidence.
4. `docs/coverage_report.md` + `docs/coverage_waiver_rationale.md` + `docs/toggle_coverage_waiver.md` — coverage closure and waiver rationale.
...
3. **Key Documents** (recommended reading order):
   ...
   | Doc | Purpose |
   |-----|---------|
   | `docs/reviewer_quickstart.md` | Three-minute evaluation route, command list, and submission boundary |
...
- 86 collected tests across smoke, directed, corner, random, and multi-seed random suites.
- The 86 figure is the pytest test count; the 91 figure in Toffee funcov is the coverage-point count, so they are intentionally different metrics.
...
- `make test-directed` covers the directed suite.
- `make test` runs smoke + directed + corner regression.
- `scripts/collect_coverage.sh` runs the full `tests/` suite and produces the Toffee/RTL coverage reports.
...
The reviewable source and report package is the top-level `competition/` directory. The important source-controlled content is:
...
Generated or local toolchain-heavy directories are intentionally separated by `.gitignore` and may be regenerated:

- `build/`
- `local/`
- `tools/`
- `cache.vcd`, `*_coverage.dat`, Python caches, and wave files

Keeping the generated directories locally is useful for fast re-runs, but the submitted evidence is preserved in `docs/`, `unity_test/`, `uc_test_report/`, and `final_report/`.
