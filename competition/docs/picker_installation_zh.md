...
```text
competition/local/picker
```

常用路径：

```text
competition/local/picker/bin/picker
competition/local/picker/share/picker/python
competition/tools/picker
```
...
```bash
cd competition/tools/picker
make init
rm -rf build
make -j$(sysctl -n hw.ncpu) ARGS="-DPython3_EXECUTABLE=/Users/zzy/Workspace/ucagent/.venv/bin/python"
make install ARGS="-DCMAKE_INSTALL_PREFIX=/Users/zzy/Workspace/ucagent/competition/local/picker -DPython3_EXECUTABLE=/Users/zzy/Workspace/ucagent/.venv/bin/python"
```
