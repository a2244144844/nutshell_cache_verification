...
上方 Toffee 汇总是最终功能覆盖闭环的权威口径。下方逐 bin 表格是 legacy random collector 在默认 seed/step 样本（`seed=7`, `steps=18`）下的采样视图；当 Toffee 汇总为 100% 时，下方标记为 `gap` 的 legacy 条目不代表 Toffee 功能覆盖仍有缺口。
