...
| Picker CLI | Available locally | Installed from source at `competition/local/picker`; see `docs/picker_installation.md`. |
| Java runtime | Available locally | Azul Zulu JRE `17.0.19` installed at `competition/local/jre17`. |
| Mill | Available locally | Mill `0.11.7` installed at `competition/local/mill/bin/mill`, matching NutShell `.mill-version`. |
