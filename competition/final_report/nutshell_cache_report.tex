...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.92\textwidth]{01_verification_flow.pdf}
  \caption{验证流程图 —— 从环境搭建到覆盖率闭环的五阶段工作流}
  \label{fig:flow}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.92\textwidth]{02_verification_arch.pdf}
  \caption{验证平台架构 —— Toffee 风格四层组件与数据通路}
  \label{fig:arch}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.92\textwidth]{03_dut_dataflow.pdf}
  \caption{DUT 数据流图 —— Cache 模块的 CPU 侧、内存侧与 MMIO 信号通路}
  \label{fig:dataflow}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.92\textwidth]{04_dut_structure.pdf}
  \caption{DUT 层次结构 —— CacheStage1/2/3 三级流水线及子模块关系}
  \label{fig:structure}
\end{figure}
