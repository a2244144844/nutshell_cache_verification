...
if toffee:
    ...
    lines.append(
        f"- **Covered Functional Coverage Points**: {toffee['covered_points']}/{toffee['points']} "
...
lines.append("")
lines.append(
    "The Toffee summary above is the authoritative functional-coverage closure metric. "
    "The per-bin tables below are the legacy random-collector view for the default "
    f"seed/step sample (`seed={seed}`, `steps={steps}`); entries marked `gap` there "
    "do not imply an uncovered Toffee bin when the Toffee summary is 100%."
)
lines.append("")
...
lines.append("## Legacy Random Collector Notes")
lines.append("")
gaps = []
