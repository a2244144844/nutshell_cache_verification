...
| Picker CLI | 本地可用 | 从源码安装于 `competition/track1_nutshell_cache/local/picker`；参见 `docs/picker_installation.md`。 |
| Java 运行时 | 本地可用 | Azul Zulu JRE `17.0.19` 安装于 `competition/track1_nutshell_cache/local/jre17`。 |
| Mill | 本地可用 | Mill `0.11.7` 安装于 `competition/track1_nutshell_cache/local/mill/bin/mill`，匹配 NutShell `.mill-version`。 |
