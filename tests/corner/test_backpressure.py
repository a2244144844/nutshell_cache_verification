...
def _mem_request_signature(env):
...
def _cpu_response_signature(env):
...
async def test_memory_request_backpressure_holds_request_until_ready(cache_env):
    env = cache_env
    env.reset()
...
@toffee_test.testcase
async def test_cpu_response_backpressure_holds_response_until_ready(cache_env):
    env = cache_env
    env.reset()
